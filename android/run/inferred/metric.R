...
plot <- 
  ggplot(data=df, aes(x=interaction,y=win,colour=policy,shape=policy)) + 
  geom_line() +
  geom_point(size=4) +
  scale_x_continuous(breaks=c(0,25,50,75,100,125), limits=c(0,140)) +
  #scale_y_continuous(limits=c(50,100)) +
  scale_color_manual(name="Run", breaks=c("r1","r2","r3","r4"), labels=c("Run-0", "Run-1", "Run-2", "Run-3"), values=pal) +
  scale_shape_manual(name="Run", breaks=c("r1","r2","r3","r4"), labels=c("Run-0", "Run-1", "Run-2", "Run-3"), values=c(1,2,3,4)) +

  #scale_color_manual(name="Run", breaks=c("r1"), labels=c("Run-1"),values=c("red3", "blue3")) +
  #scale_shape_manual(name="Run", breaks=c("r1"), labels=c("Run-1"),values=c(1,2)) +

  xlab("Interactions") +
  ylab("Energy (J)") +
  theme_minimal(base_size=16) +
  theme(legend.position="bottom") +
  geom_hline(yintercept=0, colour="red4", linetype = "longdash") 

pdf(paste(outputpdf, "win-reward.pdf",sep="/"),height=4,width=12)
print(plot)
...
#for (i in c("0", "1", "2", "3")) {
  ...
  pdf(paste(outputpdf, paste(i, "-rate-time.pdf", sep=""),sep="/"),height=3,width=12)
  print(plot)
}
