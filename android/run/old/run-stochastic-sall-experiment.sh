...
for i in `seq 0 ${samples_len}`; do
  ...
  # Setup
  echo "MAPSME_EXPERIMENT=SCENARIO=string" > $hdir/stoke_properties.txt
  ...
  echo "MAPSME_SAMPLING_POLICY=SAMPLE_ALL=string" >> $hdir/stoke_properties.txt
  echo "MAPSME_SAMPLES=$sample=int" >> $hdir/stoke_properties.txt 

  rm stochastic-sall-experiment/mapsme.$sample.log
  touch stochastic-sall-experiment/mapsme.$sample.log
  ...
  for r in `seq 0 $runs`; do
    ...
    cat /sdcard/stoke/stoke.log >> stochastic-sall-experiment/mapsme.$sample.log
