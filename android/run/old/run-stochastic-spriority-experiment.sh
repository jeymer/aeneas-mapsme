...
for i in `seq 0 ${samples_len}`; do
  ...
  rm stochastic-spriority-experiment/mapsme.$sample.log
  touch stochastic-spriority-experiment/mapsme.$sample.log
  ...
  for r in `seq 0 $runs`; do
    ...
    cat /sdcard/stoke/stoke.log >> stochastic-spriority-experiment/mapsme.$sample.log
