...
public enum LocationHelper
{
  ...
  private static final long INTERVAL_FOLLOW_AND_ROTATE_MS = 5000;
  private static final long INTERVAL_FOLLOW_MS = 500;
  ...
  private final LocationListener mCoreLocationListener = new LocationListener()
  {
    ...
    public void onLocationUpdated(Location location)
    {
      ...
      if (_lastLocation != null) {
        /*
        if (MwmActivity.experiment == Experiment.CONSTRAINED) {
          LogUtil.writeLogger(String.format("ELocation %d: Accuracy:%.2f\n", _locationCount, location.getAccuracy())); 
        }
        System.err.format("STOKE: ELocation %d: Accuracy:%.2f\n", _locationCount, location.getAccuracy()); 
        _locationCount++;
        _summedAcc += location.getAccuracy();
        */

      }
      _lastLocation = location;
      /*
      if (!MwmActivity.bandit.isFrozen()) {
        MwmActivity.bandit.updateRecording("location_accuracy", KnobValT.haveDouble((double)location.getAccuracy()));
      } else {
        System.err.format("STOKE: Frozen ELocation %d: Accuracy:%.2f\n", _locationCount, location.getAccuracy()); 
      }
      */
  ...
  {
    System.out.format("STOKE: LocationHelper initialize()\n");
    initProvider();
  ...
  private void initProvider()
  {
    ...
    {
      System.out.format("STOKE: Using fused provider\n");
      mLogger.d(TAG, "Use fused provider.");
      mLocationProvider = new GoogleFusedLocationProvider(new FusedLocationFixChecker(), mTelephonyManager);
    }
    else
    {
      System.out.format("STOKE: Using native provider\n");
      initNativeProvider();
    }
  }
  ...
  void startSensors(int sensorRate)
  {
    mSensorHelper.start(sensorRate);
  }
  ...
  public int getSensorRate() {
    return mSensorHelper.mSensorRate;
  }
  ...
  {
    /*
    if (MwmActivity.experiment == Experiment.CONFIGURATION) {
      mInterval = ((Integer) AndroidUtil.getProperty("MAPSME_INTERVAL"));
      System.out.format("STOKE: Using interval %d for configuration test\n", mInterval);
      return;
    } 
    */
    ...
    switch (mode)
    {
      case LocationState.FOLLOW:
        System.out.format("STOKE: using follow ms\n");
        mInterval = INTERVAL_FOLLOW_MS;
        break;

      case LocationState.FOLLOW_AND_ROTATE:
        System.out.format("STOKE: using follow and rotate ms\n");
        mInterval = INTERVAL_FOLLOW_AND_ROTATE_MS;
        break;

      default:
        System.out.format("STOKE: using not follow ms\n");
   ...
   */
  @UiThread
  public void attach(@NonNull UiCallback callback, TelephonyManager telephonyManager)
  {
  ...
  {
    Location lastKnownL = null;
    if (mSavedLocation != null) {
      lastKnownL = mSavedLocation;
    } else {
      lastKnownL = AndroidNativeProvider.findBestNotExpiredLocation(expirationMillis);
    }
