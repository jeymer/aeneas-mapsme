...
class SensorHelper implements SensorEventListener
{
  ...
  void start(int sensorRate)
  {
    mSensorRate = sensorRate;
    if (mSensorRate == MwmActivity.SENSOR_OFF) {
      // Special case, no sensor data
      return;
    }

    // Otherwise, use the constant provided
  ...
  {
    if (mSensorManager == null || mSensorRate == -1)
      return;
